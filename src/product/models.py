...
class Product(models.Model):
    ...
    # Rate
    rate_fequency = models.CharField(max_length=50, choices=RateFrequency.choices, null=True)
    rate_times = models.PositiveIntegerField(default=4, null=True)
    rate = models.DecimalField(max_digits=4, decimal_places=1, null=True)

    # Product
    description = models.TextField()
    buy_price = models.PositiveIntegerField()
    sell_price = models.PositiveIntegerField()  # Cron
    ...
    def save(self, *args, **kwargs):
        if not self.pk:
            self.rate_fequency = Product.RateFrequency.WEEK
            self.rate_times = 4
