...
def test_loan_create_calc(client_attendant, admin, load_all_fixtures_for_module, payload, exp_status):
    response_update = client_attendant.post(path="/product/", data=payload, format="json")
...
@pytest.mark.django_db
def test_loan_extend_calc(client_attendant, load_all_fixtures_for_module, product_id, payload, exp_status):
    response_get = client_attendant.get(path=f"/product/{product_id}/", data=payload, format="json")
    response_update = client_attendant.patch(path=f"/product/{product_id}/", data=payload, format="json")
    assert response_update.status_code == exp_status
...
@pytest.mark.django_db
def test_loan_return_calc(client_attendant, load_all_fixtures_for_module, product_id, payload, exp_status):
    response_get = client_attendant.get(path=f"/product/{product_id}/", data=payload, format="json")
    response_update = client_attendant.patch(path=f"/product/{product_id}/", data=payload, format="json")
    assert response_update.status_code == exp_status
...
def test_update_loan(
    client_attendant, load_all_fixtures_for_function, product_id, payload_data, exp_status_patch, exp_status_get
):
    response_get_prev = client_attendant.get(path=f"/product/{product_id}/")
    response_update = client_attendant.patch(path=f"/product/{product_id}/", data=payload_data, format="json")
    ...
    assert response_get.data["product_name"] != response_get_prev.data["product_name"]
    assert response_get.data["sell_price"] == response_get_prev.data["sell_price"]
    assert response_get.data["date_create"] == response_get_prev.data["date_create"]
    assert response_get.data["date_extend"] == response_get_prev.data["date_extend"]
    assert response_get.data["inventory_id"] == response_get_prev.data["inventory_id"]
